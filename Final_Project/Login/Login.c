...
//unsigned int userTypeMirror;
static unsigned int Token;
//unsigned int tokenMirror;


/********************THESE SETTER FUNCTIONS ARE ONLY FOR TESTING!!!**********************/
void setUserTypeForTesting(unsigned char type) {
	User_Type = type;
}

void setTokenForTesting(unsigned int token) {
	Token = token;
}
